...
.. note:: Arithmetic limitations
   ...
   However, **multiplication, division, and powers do not update the
   dimension label** on the resulting :class:`~unxt_coordax.DimensionAxis`.
   The result field retains the coordinate objects of the first operand
   unchanged.  This means that, for example, squaring a *length* field yields
   a field whose axis is still labelled *length* even though the values are
   now an *area*.  If you need the result dimension, compute it explicitly::
   ...
   and attach a fresh :class:`~unxt_coordax.DimensionAxis` to the result.
"""
